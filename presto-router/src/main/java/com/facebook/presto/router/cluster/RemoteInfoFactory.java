...
public class RemoteInfoFactory
{
    ...
    {
        return new RemoteQueryInfo(clusterInfoHttpClient, uriBuilderFrom(uri).appendPath(QUERY_INFO).build(), routerConfig, remoteStateConfig);
    }

    public RemoteClusterInfo createRemoteClusterInfo(URI uri)
    {
        return new RemoteClusterInfo(queryInfoHttpClient, uriBuilderFrom(uri).appendPath(CLUSTER_INFO).build(), routerConfig, remoteStateConfig);
    }
}
